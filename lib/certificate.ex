defmodule PhoenixAlexa.Certificate do
  @moduledoc """
  ...
  defp extension(cert, ext_id) do
    ext =
      ...
      |> Enum.find(fn ext ->
        elem(ext, 0) == :Extension && get_field(ext, :extnID) === ext_id
      end)
  ...
  defp validity(cert) do
    {:Validity, {:utcTime, from}, {:utcTime, to}} =
      cert
      |> get_field(:validity)
  ...
  defp check_cert_dates(certs) do
    Enum.all?(certs, fn cert ->
      ...
      NaiveDateTime.diff(now, from) > 0 && NaiveDateTime.diff(to, now) > 0
    end)
  end
